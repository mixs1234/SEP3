...
public class OrderBrokerServiceTest
{
    ...
    public async Task Create_Order_Test_Success()
    {
        ...
        Xunit.Assert.Equal("1", response.Data.ToString());
        Xunit.Assert.Equal("Order created successfully.", response.Message);
    }
    ...
    public async Task Create_Order_Test_Failure()
    {
        ...
        Xunit.Assert.Equal(errorMessage, deserializedMessage);
    }
}